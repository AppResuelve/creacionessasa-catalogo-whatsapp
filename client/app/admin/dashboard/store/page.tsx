...
export default function Store() {
  ...
  const current =
    STORE_STATUS.find((s) => s.value === status) || STORE_STATUS[0];
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-zinc-500">
            Vista previa de tu sitio público
          </p>
        </div>
      ...
      <div className="flex flex-col gap-2 p-4 rounded-xl border bg-zinc-900/50 border-zinc-800">
        ...
        <span className="text-sm text-zinc-500">
          ...
          {status === "draft" &&
            "Tu tienda está oculta. Ideal para hacer cambios."}
          {status === "maintenance" &&
            "Tu tienda está en mantenimiento. Contactanos."}
        </span>
      </div>
    </div>
  );
